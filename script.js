  ...
  .then((data) => {
    ...
    <div class="popcontainer-text">
      ...
      marker.on("popupopen", function () {
        ...
        document.querySelector(".leaflet-popup-content .ship-image").onclick =
          function () {
...
function filterMarkers() {
  ...
  if (selectedTypes.length === 0) {
    ...
    checkboxList.style.display =
      checkboxList.style.display === "none" ? "flex" : "none";
  } else {
    markers.clearLayers();
...
function resetFilters() {
  shipFilters.forEach((checkbox) => {
    checkbox.checked = false;
  });
  $('.checkbox-list-ul input[type="checkbox"]')
    .parent("label")
...
document
  .getElementById("showVesselTypesBtn")
...
function changeLabelColor() {
  $('.checkbox-list-ul input[type="checkbox"]').change(function () {
    if (this.checked) {
      $(this)
        .parent("label")
        .css("text-shadow", "1px 1px 10px #fff, 1px 1px 10px #ccc");
    } else {
      $(this).parent("label").css("text-shadow", "none");
    }
  });
}
